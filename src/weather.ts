...
export async function weatherResponse(request:Request):Promise<Response>{
  ...
  if(hit){const data=await hit.json() as Record<string,unknown>;return json({...data,location_name:roundedLat===DEFAULT_LAT&&roundedLon===DEFAULT_LON?DEFAULT_NAME:undefined},{headers:{'x-nexa-weather-cache':'HIT'}})}
  try{
    ...
    const payload=JSON.stringify({...data,location_name:roundedLat===DEFAULT_LAT&&roundedLon===DEFAULT_LON?DEFAULT_NAME:undefined});
    const response=new Response(payload,{status:200,headers:{'content-type':'application/json; charset=utf-8','cache-control':'public, max-age=300, s-maxage=300','x-nexa-weather-cache':'MISS'}});
