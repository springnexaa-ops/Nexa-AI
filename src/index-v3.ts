...
const users=new Map<string,User>(); const sessions=new Map<string,Session>(); const guestChats=new Map<string,{count:number,resetAt:number}>();
...
async function authApi(request:Request,env:any,url:URL):Promise<Response|null>{
  ...
  if(url.pathname.startsWith('/v1/admin/users')){if(!isAdmin(request,env))return json({error:'Unauthorized'},401);const suffix=url.pathname.slice('/v1/admin/users'.length).replace(/^\//,'');if(request.method==='GET'&&!suffix)return json({ok:true,persistent:false,storage:'worker-memory',users:[...users.values()].map(publicUser)});if(request.method==='POST'&&!suffix){const b:any=await request.json().catch(()=>({}));const name=clean(b.name,100),email=clean(b.email,160).toLowerCase(),role=clean(b.role,60)||'User';if(!name||!email.includes('@'))return json({error:'Name and a valid email are required.'},400);if([...users.values()].some(u=>u.email===email))return json({error:'A user with this email already exists.'},409);const now=new Date().toISOString();const u:User={id:makeId(),name,email,role,status:'pending',passwordHash:await hashPassword(crypto.randomUUID()),createdAt:now,updatedAt:now};users.set(u.id,u);return json({ok:true,user:publicUser(u)},201)}const u=users.get(suffix);if(!u)return json({error:'User not found'},404);if(request.method==='PATCH'){const b:any=await request.json().catch(()=>({}));if(b.name!==undefined)u.name=clean(b.name,100)||u.name;if(b.email!==undefined){const email=clean(b.email,160).toLowerCase();if(!email.includes('@'))return json({error:'Invalid email.'},400);u.email=email}if(b.role!==undefined)u.role=clean(b.role,60)||u.role;if(['pending','accepted','rejected'].includes(b.status))u.status=b.status;if(typeof b.password==='string'&&b.password.length>=10)u.passwordHash=await hashPassword(b.password);u.updatedAt=new Date().toISOString();for(const[t,s]of sessions)if(s.userId===u.id)sessions.delete(t);return json({ok:true,user:publicUser(u)})}if(request.method==='DELETE'){for(const[t,s]of sessions)if(s.userId===u.id)sessions.delete(t);users.delete(u.id);return json({ok:true,deleted:u.id})}}
  return null;
}
