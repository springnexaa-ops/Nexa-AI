...
async function readResponse(response){
  ...
  if(!text.trim())throw new Error(`Upload service returned an empty response (HTTP ${response.status}).`);
  try{return JSON.parse(text)}catch{
    const type=response.headers.get('content-type')||'unknown';
    throw new Error(`Upload service returned ${type} instead of JSON (HTTP ${response.status}).`);
  }
}
async function upload(input){
  const out=document.getElementById('nxOut');
  ...
  try{
    ...
    if(!response.ok||data.ok===false)throw new Error(data.error||data.message||`Upload failed (HTTP ${response.status}).`);
